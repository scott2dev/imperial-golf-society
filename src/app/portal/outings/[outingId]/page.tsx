...
export default async function OutingScoringPage({ params }: OutingPageProps) {
  ...
  const canOpenResultsPage = Boolean(outing.resultsPublishedAt) || (isCaptainOrAdmin && canShowCaptainResults);
      ...
      {canOpenResultsPage ? (
        <section className="mx-auto mt-6 max-w-6xl px-4 sm:px-6">
          <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm sm:p-8">
            ...
            <h2 className="mt-3 text-2xl font-semibold text-[var(--brand-dark)]">
              View the outing results
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-700">
              {outing.resultsPublishedAt
                ? "The results have been published and are now available to members."
