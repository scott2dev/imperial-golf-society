...
export default async function OutingResultsPage({ params }: ResultsPageProps) {
  ...
  if (!canViewResults) {
    redirect(`/portal/outings/${outingId}`);
  }
        ...
        <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm sm:p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {isCaptainOrAdmin && canShowCaptainResults && !isPublished ? (
                <ConfirmActionModal
                  action={publishOutingResults}
                  buttonLabel="Publish results"
                  buttonClassName="inline-flex min-h-10 items-center justify-center rounded-full bg-[var(--brand)] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[var(--brand-dark)]"
